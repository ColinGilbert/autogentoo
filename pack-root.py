...
# Pre-conditions that must totally be met.
...
# Now add the subtle flavours.
os.system('cp -R ' + HW_CONFIG_DIR + '/* ' + STAGING_DIR)
os.system('cp -R ' + SECRET_SAUCE_CONFIG_DIR + '/* ' + STAGING_DIR)

# We add a console because the initramfs doesn't load one by default - the Linux kernel devs only saw the initramfs as a temporary pre-userland environment but here we are, using it as the rootfs and never leaving the nest like an adult of 34 years living in their parents' basement.. 
